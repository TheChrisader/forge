...
export class GrafanaBackupStrategy implements BackupStrategy {
  ...
  async runBackup(params: BackupParams): Promise<BackupResult> {
    ...
    return {
      stream: stdout,
      extension: "tar.gz",
      metadata: { format: "tar-gzip", dataPath: "/var/lib/grafana" },
    };
  }
  ...
  async runRestore(params: RestoreParams): Promise<void> {
    ...
    const restoreResult = await runtime.exec(
      containerId,
      ["tar", "xzf", "/tmp/forge-restore.tar.gz", "-C", "/var/lib/grafana"],
      { attachStdout: true, attachStderr: true }
    );
