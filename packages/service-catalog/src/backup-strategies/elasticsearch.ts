...
export class ElasticsearchBackupStrategy implements BackupStrategy {
  ...
  async runBackup(params: BackupParams): Promise<BackupResult> {
    ...
    const { stdout, wait } = await runtime.execStream(
      containerId,
      ["tar", "czf", "-", "-C", "/usr/share/elasticsearch/data", "."],
    ...
    return {
      stream: stdout,
      extension: "tar.gz",
      metadata: { format: "tar-gzip", dataPath: "/usr/share/elasticsearch/data" },
    };
  }
  ...
  async runRestore(params: RestoreParams): Promise<void> {
    ...
    const restoreResult = await runtime.exec(
      containerId,
      ["tar", "xzf", "/tmp/forge-restore.tar.gz", "-C", "/usr/share/elasticsearch/data"],
      { attachStdout: true, attachStderr: true }
    );
